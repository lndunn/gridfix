...
def params(scenario, fleet_size=10000, seed=0):
    
    hyperpriors = {}
    keys = pd.DataFrame([key.split('_') for key in list(scenario.keys())]).set_index(0)
    
    params = pd.DataFrame(columns=list(set([key.split('_')[0] for key in list(scenario.keys())])), 
                          index=list(range(fleet_size)))
...
def failure_data(params, X, link, seed=0, fleet_size=1000):
    ...
    n_components = params.groupby(list(params.keys()).tolist())[list(params.keys())[0]].count()
    p_labels = list(params.keys())
    params = params.set_index(list(params.keys()).tolist())
    ...
    for p_vals in params.index.unique():
        failure_prob = pd.Series(link().failure_prob(dict(list(zip(p_labels, p_vals))), X), index=X.index)
